/**
 * YamMaster - Serveur de jeu WebSocket
 */
...
const createGame = (player1Socket, player2Socket) => {
  ...
  // Ajout de la partie à la liste des parties actives
  games.push(newGame);

  let gameIndex = GameService.utils.findGameIndexById(games, newGame.idGame);

  // Notification du début de partie aux joueurs
  games[gameIndex].player1Socket.emit('game.start', GameService.send.forPlayer.viewGameState('player:1', games[gameIndex]));
  games[gameIndex].player2Socket.emit('game.start', GameService.send.forPlayer.viewGameState('player:2', games[gameIndex]));

  // Mise à jour des vues initiales
...
io.on('connection', socket => {
  console.log(`[${socket.id}] Nouvelle connexion établie`);
